...
use crate::{
    ...
    txn_validator::{self, TxnValidator},
};
...
impl Core {
    ...
    //         Ok(state) => return state.clone(),
    //         Err(_) => {
    //             self.send_err_msg(CoreError::FailedToAcquireStateLock);
    //             CoreState::Inactive
    //         }
    //     }
    // }
    // fn set_state(&self, new_state: CoreState) {
    //     match self.state.lock() {
    //         Ok(mut state) => {
    //             *state = new_state;
    //         }
    //         Err(_) => self.send_err_msg(CoreError::FailedToAcquireStateLock),
    //     }
    // }
    ...
    pub fn new<D: Database>(
        id: CoreId,
        error_sender: Sender<(CoreId, CoreError)>,
        state_read_handle: ReadHandle<InnerTrie<D>>,
    ) -> Self {
        Self {
            join_handle: None,
            control_sender: None,
            error_sender,
            // state: Arc::new(Mutex::new(CoreState::Ready)),
            id,
        }
    }
...
impl<D: Database + 'static> ValidatorUnit<D> {
    ...
    pub fn new(
        mempool_read_handle: ReadHandle<Mempool>,
        // TODO: Use this (I assume the state Trie will be required for proper validation)
        state_trie_read_handle_factory: ReadHandleFactory<InnerTrie<D>>,
        mempool_processor_sender: Sender<MempoolControlMsg>,
        amount_of_cores: u8,
        ...
        for i in 0..amount_of_cores {
            ...
            cores.push(Core::new(
                i,
                core_error_channel_sender.clone(),
                state_trie_read_handle_factory.handle(),
            ));
        }
        Self {
            cores,
