...
// pub fn generate_key_sets(number_of_nodes: u16) -> (Vec<SecretKey>,
// BTreeMap<NodeId, PublicKey>) {     let sec_keys: Vec<SecretKey> =
// (0..number_of_nodes).map(|_| rand::random()).collect();     let pub_keys =
// sec_keys         .iter()
...
// /// * `node_type`: NodeType - This is the type of node that we want to
// generate. ///   We can choose from the
// /// following:
// ///
// /// Returns:
// ///
// /// A DkgEngine struct with a node_info field that is an Arc<RwLock<Node>>.
// pub async fn generate_dkg_engines(total_nodes: u16, node_type: NodeType) ->
// Vec<DkgEngine> {     let (sec_keys, pub_keys) =
// generate_key_sets(total_nodes);     let mut dkg_instances = vec![];
//     for i in 0..total_nodes {
//         let secret_key: SecretKey = sec_keys.get(i as
// usize).unwrap().clone();         let _secret_key_encoded =
// bincode::serialize(&SerdeSecret(secret_key.clone())).unwrap();
//
//         // let (_, msg_receiver) = unbounded_channel::<(Packet,
//         // std::net::SocketAddr)>(); let (msg_sender, _) =
// unbounded_channel();
//
...
// dkg_engine_node1.generate_sync_keygen_instance(1).unwrap();
//     let part_committment_node2 =
// dkg_engine_node2.generate_sync_keygen_instance(1).unwrap();
//     let part_committment_node3 =
// dkg_engine_node3.generate_sync_keygen_instance(1).unwrap();
//     let part_committment_node4 =
