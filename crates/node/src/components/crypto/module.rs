...
pub struct DkgModuleConfig {
    ...
    pub secret_key: hbbft::crypto::SecretKey,
    pub rendezvous_local_addr: SocketAddr,
    pub rendezvous_server_addr: SocketAddr,
    pub quic_port: u16,
    pub broadcast_events_tx: EventPublisher,
    ...
    fn process_request(
        ...
        if let RendezvousRequest::Ping = request {
            ...
            if let Ok(data) = bincode::serialize(&response) {
                if let Err(err) = sender.send(Packet::reliable_unordered(packet.addr(), data)) {
                    telemetry::error!("{:?}", err);
                }
            }
        };
...
// impl Default for DkgModuleConfig {
//     fn default() -> Self {
//         Self {
//             quorum_type: Default::default(),
...
//             secret_key: todo!(),
//             config: todo!(),
//             rendezvous_local_addr: todo!(),
//             rendezvous_server_addr: todo!(),
//             quic_port: todo!(),
//             broadcast_events_tx: todo!(),
//         }
//     }
// }
