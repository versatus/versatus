...
#[tokio::test]
async fn network_can_apply_genesis_block() {
    ...
    for node in nodes.iter_mut() {
        dbg!(node.read_handle().state_store_values());
    }

    // let mut node = nodes.get_mut(3).unwrap();
    let mut node = nodes
        // .last_mut(|node| node.node_type == NodeType::Miner)
        .last_mut()
        .unwrap();
    ...
    let res = rpc_client.get_full_state().await.unwrap();
