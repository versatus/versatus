use std::{
    path::Path, process, 
    borrow::Borrow, net::{IpAddr, Ipv4Addr}
};
...
impl BrokerAddr {
    ...
    pub fn new_from_str(
        ...
        let local_port = match port.parse::<u16>() {
            Ok(port) => port,
            Err(_) => 9000 as u16,
        };
...
pub struct AppContext<'m> {
    ...
    pub broadcast_target_address: String,  // "255.255.255.255" or specific sub network 192.168.255.255
        ...
        where AppContext<'m>: Send + 'static
    {
        ...
        RcCell::new(
            AppContext {
                ...
                node_id,
                node_state: if config.is_node_origin { NodeUpdateState::UpToDate } else {NodeUpdateState::InProgress},
