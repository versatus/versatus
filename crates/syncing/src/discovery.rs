...
pub fn route_table_cleaning_routine_start(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            ctx.get().borrow_mut().node_routes_db.remove_inactive_nodes(MAX_TIMEOUT_SECS);
        }
    });
}
...
pub fn node_discoverer_start(
    ...
    info!("node_discoverer_start: Discoverer started on {}", context.get().borrow().bind_sender_local_address.as_str());
    ...
    tokio::spawn(async move {
        ...
        match Ipv4Addr::from_str(broadcast_target_address.as_str()) {
                    ...
                    ).await
                {
                    ...
                    loop {
                        ...
                        if let Some(cluster) = ctx.get().borrow_mut().node_routes_db.get_cluster(NodeType::ALL) {
                            ...
                            cluster_ipv4s.sort_by(|a, b| a.cmp(&b));
                            cluster_ipv4s.dedup();
                            message_template.cluster_ipv4 = cluster_ipv4s;
    
                            let mut cluster_ipv6s: Vec<Ipv6Addr> = 
                                cluster
                                    .into_iter()
                                    .flat_map(|n| 
                                        Vec::from_iter(n.addr_set)
                            ...
                            cluster_ipv6s.sort_by(|a, b| a.cmp(&b));
                            cluster_ipv6s.dedup();
            ...
            Err(e) => {
                error!("node_discoverer_start: Cannot parse the specified address {} : {}", &ctx.get().borrow().broadcast_target_address.clone(), e);
            }
        }
    });
    ...
    tokio::spawn(async move {
        ...
        match Ipv4Addr::from_str(bind_sender_local_address.as_str()) {
            ...
            Err(e) => {
                error!("Cannot parse the specified address {} : {}", context.get().borrow().bind_sender_local_address.clone(), e);
            }
        }
