...
pub async fn node_bootstrap_syncing_context_start(
    ...
    let mut node_route_message = NodeRouteEntry::new(
        context.get().borrow().node_id.clone().to_string(),
        NodeType::Archive,
    ...
    let mut bootstrap_context = BootstrapContext::init(context.get().borrow().node_state == NodeUpdateState::UpToDate);
    ...
    received_messages.for_each(move |m| {
        ...
        context .get()
                .borrow_mut()
                    .node_routes_db
                        .add_node(m.node_id.clone(), if node_distance_in_millis > 0 { node_distance_in_millis } else { 0 }, m.node_type, m.ip())
            ...
            m.node_type == node_type {
                    ...
                    match broker_retrieve(context.clone(), m.ip()) {
                        Ok(retrieved_bytes) => {
                            ...
                            info!("Node was correctly bootstrapped with {} bytes.", retrieved_bytes);

                            bootstrap_context.is_local_node_boostrapped = true;
                            context.get().borrow_mut().set_state(NodeUpdateState::UpToDate);
                            ...
                            broker_server_start(
                                context.clone(),
                                offset_localstate_in_file
                            );
                        },
                        Err(e) => {

                            bootstrap_context.is_local_node_boostrapped = false;
                            context.get().borrow_mut().set_state(NodeUpdateState::Invalid);
