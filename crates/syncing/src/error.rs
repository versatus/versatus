...
impl fmt::Display for BootstrapError {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            BootstrapError::NodeMissing => {
                write!(f, "Connected node went missing during synchronization")
            },
            BootstrapError::NodeOutOfSync => write!(f, "This node is out of sync as of now"),
            BootstrapError::GeneralConnectionError => {
                write!(f, "Connection Error - network problems.")
            },
        }
    }
}
