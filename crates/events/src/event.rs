...
use primitives::{
    AckBytes, Address, CurrentNodeId, Epoch, FarmerQuorumThreshold, NodeId, NodeIdx,
    PartCommitmentBytes, PublicKeyShareVec, RawSignature, Round, Seed, SenderId,
};
...
pub enum Event {
    ...
    /// `PartMessage(SenderId, PartCommitmentBytes)` is an event that is triggered when part
    ...
    /// `SendPartMessage(SenderId, PartCommitmentBytes)` is an event that is triggered when part
    /// message has been received from the network, and has to be recorded
    /// in the part message store.
