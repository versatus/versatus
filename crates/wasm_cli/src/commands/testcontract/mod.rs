...
pub fn run(opts: &TestContractOpts) -> Result<()> {
    ...
    .args(&opts.args);
    wasm.execute()?;
    let updated_contract_inputs: &SmartContractInputs = &serde_json::from_str(&wasm.stdout())?;
    // TODO: Use these inputs to update database Storage mentioned below.
    let _updated_balance = updated_contract_inputs.account_info().account_balance;
    let _updated_metadata = &updated_contract_inputs.protocol_input;
...
// fn update_documents(storage_connection: &Storage, opts: &TestContractOpts) -> Result<()> {
//     let new_document = &create_contract_inputs(&opts.function, &opts.inputs, &opts.dbpath);
//     let old_document = storage_connection;
