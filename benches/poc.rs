...
fn xor_hash(seed: &u64, hash: &U256) -> U256 {
    ...
    hash.0
        .iter()
        .enumerate()
        .for_each(|(idx, inner)| res[idx] = inner ^ seed);

    U256(res)
}

fn xor_claim_hashes(seed: &u64, claim_hashes: &[U256]) -> Vec<U256> {
    claim_hashes
        .iter()
        .map(|hash| xor_hash(seed, hash))
        .collect()
}
...
pub fn ten_iter_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("ten_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}

pub fn hundred_iter_benchmark(c: &mut Criterion) {
    let (seed, claim_hashes) = iter_setup(100);
    c.bench_function("hundred_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}
...
pub fn thousand_iter_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("thousand_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}
...
pub fn ten_thousand_iter_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("ten_thousand_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}
...
pub fn hundred_thousand_iter_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("hundred_thousand_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}
...
pub fn million_iter_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("million_iter_poc", |b| {
        b.iter(|| xor_claim_hashes(black_box(&seed), black_box(&claim_hashes)))
    });
}
