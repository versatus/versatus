...
/// `DkgState` is a struct that contains all the state that is needed to run the DKG protocol.
///
/// Properties:
/// * `part_message_store` is a hashmap that stores the `Part` messages that are received from other nodes.
/// * `ack_message_store` is a hashmap that stores the `Ack` (acknowledge part commitment) messages that are received from other
/// nodes.
/// * `peer_public_keys` is a BTreeMap that stores the public keys of the other nodes.
/// * `public_key_set`: This is the set of public keys that are generated by the DKG protocol.
/// * `secret_key_share`: This is the secret key share that is generated by the DKG protocol.
/// * `sync_key_gen`: This is the key generator that will be used to generate the secret key.
/// * `random_number_gen`: This is a random number generator that is used to generate random numbers for the DKG protocol.
